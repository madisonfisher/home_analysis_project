...
function createMap() {
  ...
  d3.json("../static/json/latlong_cities.json").then((data5) => {
    for (let j = 0; j < 96; j++) {
      d3.json("../static/json/city_mean_pending_weekly_predicted_modified.json").then((data1) => {
        var pred_mean_days = [];
        for (var i = 0; i < 204; i++) {
        ...
        d3.json("../static/json/city_median_pending_weekly_predicted_modified.json").then((data2) => {
          ...
          var trace2 = {
            ...
            d3.json("../static/json/city_mean_pending_weekly_smooth_base_modified.json").then((data3) => {
            var mean_days = [];
            for (var i = 0; i < 182; i++) {
            ...
            d3.json("../static/json/city_median_pending_weekly_smooth_base_modified.json").then((data4) => {
              var median_days = [];
              for (var i = 0; i < 182; i++) {
